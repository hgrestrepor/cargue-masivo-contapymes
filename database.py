...
from procesador_excel import DEFAULT_JSON_HIJO, DEFAULT_JSON_PADRE, DEFAULT_MAPPING
...
def get_config_json():
    ...
    cursor.execute(
        "SELECT contenido, contenido_hijo, mapeo, ep_ip, ep_puerto, ep_email, ep_password, ep_iapp, ep_idmaquina, ep_keyagente, contenido_transporte, mapeo_transporte "
    ...
    if not row:
        cursor.execute(
            "INSERT INTO config_json (id, contenido, contenido_hijo, mapeo, contenido_transporte, mapeo_transporte) VALUES (1, ?, ?, ?, ?, ?)",
            (default_padre, default_hijo, default_mapeo, default_padre, default_mapeo),
        )
        conn.commit()
        return {
            ...
            "json_transporte": default_padre, "mapeo_transporte": default_mapeo,
        }
    else:
        ...
        if not mapeo or mapeo.strip() == "[]":
            mapeo = default_mapeo
            actualizar = True
        if not contenido_transporte or contenido_transporte.strip() == "{}":
            contenido_transporte = default_padre
            actualizar = True
        if not mapeo_transporte or mapeo_transporte.strip() == "[]":
            mapeo_transporte = default_mapeo
            actualizar = True
        if actualizar:
            cursor.execute(
                "UPDATE config_json SET contenido = ?, contenido_hijo = ?, mapeo = ?, contenido_transporte = ?, mapeo_transporte = ? WHERE id = 1",
                (contenido, contenido_hijo, mapeo, contenido_transporte, mapeo_transporte),
            )
            conn.commit()
    conn.close()
    return {
        ...
        "json_transporte": contenido_transporte, "mapeo_transporte": mapeo_transporte,
    }


def save_config_json(contenido, contenido_hijo="{}", mapeo="[]", endpoint=None,
                     contenido_transporte="{}", mapeo_transporte="[]"):
    conn = get_connection()
    cursor = conn.cursor()
    if endpoint:
        cursor.execute(
